...
import { DEMO_ACCOUNTS, getDemoDirectoryPeople } from "./authService.js";
import {
...
export default function AdminControlsModal({ open, onClose, session, accessToken = "", onAdminAction }) {
  ...
  const handleAddMovie = async () => {
    ...
    setCatalogMsg("Movie saved to local admin catalog (mock). Sync with API when logged in with JWT.");
    setMovieTitle("");
    setMovieGenre("");
    setMoviePosterKey("");
    setMovieNotes("");
  ...
  return (
    ...
    >
      ...
      <div className="admin-modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="admin-modal-section">
          <h3>Remove group by ID (local mock)</h3>
          <p className="admin-modal-p">
            Deletes a group from the mock store using its <strong>group ID</strong> (see list below or
        ...
        <div className="admin-modal-section">
          ...
          <p className="admin-modal-p">
            ...
            notes. With JWT, POST <code>/api/v1/movies/catalog</code>; otherwise local mock list.
          </p>
          <div className="admin-modal-row">
            <input
              className="admin-modal-input"
        ...
        <div className="admin-modal-section">
          ...
          <p className="admin-modal-p" style={{ fontSize: "12px" }}>
            Directory IDs (Friends / ban): match <code>getDemoDirectoryPeople()</code> in the app.
          </p>
          <table className="admin-demo-table">
            ...
            <tbody>
              {DEMO_ACCOUNTS.map((row, i) => {
                const dir = getDemoDirectoryPeople()[i];
                return (
                  <tr key={row.username}>
                    <td>
                      <code>{dir?.id ?? "—"}</code>
