...
export async function getInviteCandidates(req: Request, res: Response): Promise<void> {
  const { groupId } = req.params;
  const q = String(req.query.q ?? '');
  const candidates = await groupService.getInviteCandidates(groupId, req.userId!, q);
  res.json({ candidates });
}

export async function createInvite(req: Request, res: Response): Promise<void> {
  const { groupId } = req.params;
  await groupService.createGroupInvite(groupId, req.userId!, req.body.inviteeUserId);
  res.status(204).send();
}

export async function removeMember(req: Request, res: Response): Promise<void> {
  const { groupId, userId } = req.params;
  await groupService.removeMember(groupId, userId, req.userId!, req.systemRole ?? 'USER');
  res.status(204).send();
}
